...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        stats['total'] = total
        trans_df = pd.read_csv(TRANS_CSV, na_filter=False)
        for candidate in tqdm(candidates.iterator(), total=total):
            ...
            thema_codes = self.translate_codes(bic_codes, trans_df, stats)
            if not thema_codes:
                continue
    ...
    def translate_codes(self, bic_codes: list, df: pd.DataFrame, stats: Counter) -> set:
        thema_codes = set()
        for bic in bic_codes:
            rows = df[df['BIC Subject Code is exactly'] == bic]
            if len(rows) > 1:
                s = rows['Thema Subj 1'].str.len().sort_values().index
                rows.reindex(s)
            try:
                vals = rows[['Thema Subj 1', 'Thema Subj 2', 'Thema Subj 3']].values[0]
            except IndexError:
                vals = []
                stats['missing'] += 1
            vals = [x for x in vals if x]
            thema_codes |= set(vals)
        return thema_codes
